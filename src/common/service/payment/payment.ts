...
export class PaymentService {
    ...
    // Create Section Checkout
    
    async checkoutSction({
    ...
    async createPaymentMethod( data: Stripe.PaymentMethodCreateParams){
        const mehtod = await this.stripe.paymentMethods.create( data )
        return mehtod
    }
    ...
    async createRefound( id:string){
        const intent = await this.retrievePaymentIntent(id)
        if(!intent){
            throw new BadRequestException(' Invalid payment Intent')
        }
        const refound= await this.stripe.refunds.create({
            payment_intent:id
        } )
